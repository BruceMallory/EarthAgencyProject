...
setwd("C:/Users/rong.li/Desktop/新建文件夹/Lizette")
...
![(BioJftotal Model Fit Result)](C:/Users/rong.li/Desktop/新建文件夹/Lizette/fit_result.png)
...
![(Biojtscore vs. Antjtscore (Child))](C:/Users/rong.li/Desktop/新建文件夹/Lizette/child_group.png)
![(Biojtscore vs. Antjtscore (Adult))](C:/Users/rong.li/Desktop/新建文件夹/Lizette/adult_group.png)
...
The result we got when taking the MeanSever as continuous.
```{r}
fit<-lm(MeanSever ~  Condition + SRFactsTotal +invitalscore  + inpsychscore + FirstLang+Age, data = AC_df)
summary(fit)
```

When we take Meansever as continious,we got the same result as we take it as categorical--We still did not find any evidence to conclude that 'condition' had any effectbut 'Age' had significant effect on Meansever  (the p-value of Condition is bigger then 0.05,and the p-value of Age is smaller than 0.05.).
