...
## 4d.  What if we combine BioJtscore and AntJtscore (new variable `B_AJtscore`) along with combining invital and inpsych (new variable `postqs`)??  
...
ggplot(AC_df, aes(postqs, B_AJtscore, color=Age)) + geom_point() + geom_jitter() + theme_bw() + geom_smooth()
ggplot(AC_df, aes(postqs, B_AJtscore)) + geom_point() + geom_jitter() + theme_bw() + geom_smooth()
```
...
#-----
#Calculate a difference score (comBioJtscore - comAntJtscore) for everybody
AC_df$BioMinusAnthro <- as.integer(AC_df$comBioJFtotal) - as.integer(AC_df$comAntJtscore)

hist(AC_df$BioMinusAnthro   )
  ...
  theme_minimal(base_size=8) +
  #theme(legend.position="none") +
  labs(title="Biocentric - Anthropocentric for each Age") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))


#Calculate % in each of the BioMinusAnthro outcomes for children
Age1_df <- AC_df %>% 
  filter(Age=="Child") %>% 
  group_by(BioMinusAnthro, Age) %>% 
  dplyr::summarise(n=n()) %>% 
  mutate(prop=n/sum(n))

#Calculate % in each of the BioMinusAnthro outcomes for Adults
Age2_df <- AC_df %>% 
  filter(Age=="Adult") %>% 
  group_by(BioMinusAnthro, Age) %>% 
  dplyr::summarise(n=n()) %>% 
...
gg.5 <- ggplot(Age_df, aes(x=BioMinusAnthro, y=prop, fill=Age)) +
  geom_col(color="white", position = position_dodge2(width = 0.001, preserve = "single")) +
  labs(x="score", y="proportion") +
  coord_cartesian(ylim=c(0, 1), xlim=c(-3,3)) +
  theme_minimal(base_size=8) +
...
grid.arrange(gg.1, gg.2, gg.3, gg.4, gg.5, ncol=2)
```
\newpage
